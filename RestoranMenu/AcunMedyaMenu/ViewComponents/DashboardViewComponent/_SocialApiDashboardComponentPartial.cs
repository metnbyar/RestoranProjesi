        ...
        private async Task<int> GetFollowersCountAsync(string username)
        {
            try
            {
                ...
                using (var response = await client.SendAsync(request))
                {
                    ...
                    var totalToken = json["data"]?["total"];
                    if (totalToken != null && int.TryParse(totalToken.ToString(), out int followers))
                    {
                        return followers;
                    }
                    else
                    {
                        Console.WriteLine("Followers data not found or invalid.");
                        return 0;
                    }
                }
            }
            ...
            try
            {
                ...
                using (var response = await client.SendAsync(request))
                {
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        // Null kontrolü ve tip dönüşümü
                        var totalToken = json["data"]?["total"];
                        if (totalToken != null && int.TryParse(totalToken.ToString(), out int following))
                        {
                            return following;
                        }
                        else
                        {
                            Console.WriteLine("Following data not found or invalid.");
                            return 0;
                        }
                    }
                    else
